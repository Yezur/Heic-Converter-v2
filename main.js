...
const startConvertButton = document.getElementById('startConvert');
...
const convertVideo = async (item) => {
  ...
  ffmpeg.FS('readdir', '/').forEach((entry) => {
    if (entry !== '.' && entry !== '..') {
      try {
...
const applyOrientation = (canvas, ctx, data, width, height, orientation) => {
